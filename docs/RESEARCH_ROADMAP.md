...
| 3. Annotation preparation, double labeling, adjudication | **Not complete** | Resolve pooled-ID collisions [#6](https://github.com/yogesh0224/shadowleak/issues/6); complete independent blinded human annotation and freeze gold labels [#7](https://github.com/yogesh0224/shadowleak/issues/7). |
...
- [ ] Correct *pooled* annotation ID collisions: four independently generated queues contain 7,440 rows in total but reuse 1,860 anonymous IDs. Generate 7,440 unique pooled IDs and a separate private reversible model/original-ID key, preserving all original response text. [Issue #6](https://github.com/yogesh0224/shadowleak/issues/6).
- [ ] Validate cardinality, unique IDs, randomization, no missing responses and no exposed model/defense/case metadata; document residual unblinding risk if refusal wording reveals defense condition.
- [ ] Appoint two independent human annotators and an adjudicator; perform rubric calibration on separate pilot data.
- [ ] Obtain two independent labels per successful confirmatory response (up to 14,880 separate decisions), report pre-adjudication raw agreement and Cohen's kappa, adjudicate disagreements and freeze labels with hashes. [Issue #7](https://github.com/yogesh0224/shadowleak/issues/7).
